...
| `✓ 1/2` | レビューを頼んだ人のうち、何人が承認したか。色は GitHub の判定（緑＝承認済み、赤＝変更要求、琥珀＝レビュー待ち）。人数は人で数えるので、同じ人が 2 回承認しても 1 |
| 承認数の隣のアバター | レビュアー。縁が緑なら承認、赤なら変更要求、薄いままならまだ答えていない。ホバーで `mona — approved` のように出る。チームへの依頼はアバターがないので頭文字を描く。5 人目からは `+3` とまとめる |
| `↻ re-review` | 一度レビューした人に、もう一度依頼が出ている |
| `✎ draft review` | 自分がレビューを書きかけて submit していない。GitHub 上では自分にしか見えない |
...
カードのボタンやリンクでないところをクリックすると、右からパネルが出て本文が読めます。閉じるのは `✕`・Esc・パネルの外のどこかです。右上の `⧉` を押すと、その issue や pull request の GitHub の URL をクリップボードにコピーします。

本文は GitHub が組んだ HTML をそのまま受け取ります。issue や pull request 1 件ぶんを開いたときにだけ取りに行き、一度読んだものはリロードするまで取り直しません。

本文の下にやり取りが続きます。会話のコメントと、本文のあるレビューを新しい方から 20 件です。レビューには `approved` / `requested changes` が付きます。20 件を超えた分は「他に N 件」と出します。承認だけで何も書かれていないレビューは、カードの承認数に出ているので省きます。

パネルの右側には assignee・ラベル・type・マイルストーン、pull request ならレビュアーとその状態、そして親子・ブロッカー・紐づく pull request が並びます。こちらは最初のロードで取得済みのものだけなので、追加のリクエストは発生しません。
