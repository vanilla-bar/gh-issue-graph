...
本文は GitHub が組んだ HTML をそのまま受け取ります。issue や pull request 1 件ぶんを開いたときにだけ取りに行き、一度読んだものはリロードするまで取り直しません。パネルの右側には assignee・ラベル・type・マイルストーンと、親子・ブロッカー・紐づく pull request が並びます。こちらは最初のロードで取得済みのものだけなので、追加のリクエストは発生しません。
